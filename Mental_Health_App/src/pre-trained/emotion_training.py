...
# Main Trainer
# ----------------------------
class EmotionAnalyserTrainer(AnalyserTrainer):
    def __init__(self):
        self.raw_data = None
    ...
    def load_data(self):
        os.environ["HF_HUB_DISABLE_SYMLINKS_WARNING"] = "1" #Ignore HuggingFace notice
        dataset = load_dataset("dair-ai/emotion", name="unsplit", split="train")
        label_names = dataset.features["label"].names

        # Convert labels to string names
        self.raw_data = [{"text": row["text"], "label": label_names[row["label"]]} for row in dataset]
    ...
    def split_data(self):
        ...
        self.train_loader = DataLoader(EmotionDataset(X_train, y_train), batch_size=16, shuffle=True)
        self.val_loader = DataLoader(EmotionDataset(X_val, y_val), batch_size=16)
        self.test_loader = DataLoader(EmotionDataset(X_test, y_test), batch_size=16)
